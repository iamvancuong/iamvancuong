...
| `bash: node: command not found` ở Terminal | Node nằm trong virtualenv của cPanel, không có trên PATH của shell thường. Nạp trước:<br>`source ~/nodevenv/iamvancuong/*/bin/activate`<br>`deploy.sh` **tự làm việc này** từ 12/08/2026. |
| Sửa chính `deploy.sh` rồi deploy → khối mới **không chạy**, mà vẫn báo "✓ Xong" | ⭐ `git pull` ghi đè chính file đang chạy. Bash đọc script **theo vị trí byte**, không nạp cả file vào bộ nhớ — file bị thay giữa chừng thì phần còn lại đọc từ file MỚI tại vị trí byte CŨ, nhảy vào giữa dòng hoặc bỏ qua nguyên một khối. Không có lỗi nào hiện ra. Đã xử bằng cách kéo code xong thì `exec` nạp lại chính mình từ đầu (12/08/2026). **Deploy lần đầu sau khi sửa `deploy.sh` luôn đáng nghi — đọc kỹ output xem đủ các bước chưa.** |
