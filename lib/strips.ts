...
/**
 * Ảnh mẫu — chỉ dùng khi một dải CHƯA có tấm nào.
 *
 * Trang chủ thiếu hẳn một mục thì không đánh giá được bố cục. Sáu file nằm ở
 * `public/images/home/`; thay ảnh thật chỉ cần **ghi đè đúng tên file**, không
 * phải sửa code. Kích thước ghi cứng vì tỉ lệ phải biết TRƯỚC khi ảnh tải xong,
 * đợi ảnh mới biết thì cả dải giật một nhịp lúc tải.
 *
 * Có tấm thật đầu tiên là ảnh mẫu biến mất hết — không trộn lẫn, vì trộn thì
 * không nhìn ra tấm nào là mẫu.
 */
function samples(from: number, count: number, href: string): StripItem[] {
  const SIZES = [
    [1600, 1067],
    [1067, 1600],
    [1400, 1400],
...
export const SAMPLE_JOURNEY = samples(1, 3, "/journey");
export const SAMPLE_BLOG = samples(4, 3, "/blog");
